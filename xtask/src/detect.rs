...
pub fn detect_serial_ports() -> Option<(String, UsbPortInfo)> {
    ...
    if ans.len() == 0 {
        ...
        let (port_name, info) = 'outer: loop {
            ...
            println!("Input '{}' does not match to any ports! Please input again.", input);
        };
...
pub fn dump_port(port_name: &str, info: &UsbPortInfo) {
    print!("Port {}: vid: {:x}, pid: {:x}", port_name, info.vid, info.pid);
...
pub fn save_to_file(port_name: &str) {
    ...
    let mut file = fs::OpenOptions::new().read(true).write(true).create(true)
        .open(project_root().join("target").join("xtask").join("serial-port.txt"))
...
pub fn read_serial_port_choose_file() -> io::Result<String> {
    fs::read_to_string(project_root().join("target").join("xtask").join("serial-port.txt"))
}
